...
typically not expected to be used in U-mode and on many systems may immediately
or fairly quickly cause an illegal instruction exception if used at U-mode.
Unlike `WFI`, `WRS.NTO` is expected to be used by software in U-mode when
waiting on memory but without itself having a deadline for that wait to
complete.
====
